...
<body class="bg-gradient-to-br from-orange-50 via-white to-orange-100 font-poppins pt-28">
  ...
  <div class="max-w-6xl mx-auto bg-white/90 backdrop-blur-sm rounded-3xl shadow-xl overflow-hidden mb-16 border border-orange-100">
    ...
    <div class="bg-gradient-to-r from-orange-500 via-orange-400 to-orange-300 text-white px-8 py-6 flex flex-col md:flex-row items-center justify-between gap-4">
      <h1 class="text-3xl font-bold tracking-wide drop-shadow-md">Daftar Kategori</h1>
        ...
        <tbody class="divide-y divide-gray-100 bg-white">
          @foreach ($kategori as $k)
            <tr class="hover:bg-orange-50/70 transition-all duration-300">
              <td class="px-6 py-4 font-semibold text-gray-800">{{ $k->nama_category }}</td>
              <td class="px-6 py-4 text-gray-600">{{ $k->keterangan }}</td>
              ...
              <td class="px-6 py-4 text-center">
                @if($k->gambar)
                  <img src="{{ asset('storage/' . $k->gambar) }}" 
                       alt="Gambar Kategori" 
                       class="w-16 h-16 object-cover mx-auto rounded-xl shadow-md hover:scale-110 hover:shadow-lg transition-all duration-300">
                @else
              ...
              <td class="px-6 py-4 text-center">
                @if($k->status)
                  <span class="px-3 py-1 text-sm rounded-full bg-green-100 text-green-700 font-semibold">Aktif</span>
                @else
                  <span class="px-3 py-1 text-sm rounded-full bg-red-100 text-red-700 font-semibold">Non-aktif</span>
                @endif
              </td>
              ...
              <td class="px-6 py-4 text-center flex items-center justify-center gap-2">
                ...
                <a href="{{ route('kategori.edit', $k->id_category) }}" 
                ...
                <form action="{{ route('kategori.destroy', $k->id_category) }}" method="POST" class="inline-block">
                  @csrf
                  @method('DELETE')
                  <button type="submit"
