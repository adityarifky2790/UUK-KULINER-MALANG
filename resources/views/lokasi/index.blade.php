...
<body class="bg-gradient-to-br from-orange-50 via-white to-orange-100 font-poppins pt-28">
  ...
  <div class="max-w-6xl mx-auto bg-white/90 backdrop-blur-sm rounded-3xl shadow-xl overflow-hidden mb-16 border border-orange-100">
    ...
    <div class="bg-gradient-to-r from-orange-500 via-orange-400 to-orange-300 text-white px-8 py-6 flex flex-col md:flex-row items-center justify-between gap-4">
      <h1 class="text-3xl font-bold tracking-wide drop-shadow-md">Daftar Lokasi</h1>
      ...
      <table class="w-full border-collapse text-left text-sm">
        <thead class="bg-orange-100/80 text-orange-700 uppercase text-xs tracking-wider">
          <tr>
            ...
            <th class="px-6 py-4 font-semibold">Nama Daerah</th>
        ...
        <tbody class="divide-y divide-gray-100 bg-white">
          @foreach ($lokasi as $l)
            <tr class="hover:bg-orange-50/70 transition-all duration-300">
              ...
              <td class="px-6 py-4 font-semibold text-gray-800">{{ $l->nama_daerah }}</td>
              <td class="px-6 py-4 text-gray-600">{{ $l->alamat_lengkap }}</td>
              <td class="px-6 py-4 text-gray-600">{{ $l->keterangan }}</td>
              ...
              <td class="px-6 py-4 text-center flex items-center justify-center gap-2">
                ...
                <!-- Hapus -->
                <form action="{{ route('lokasi.destroy', $l->id_lokasi) }}" method="POST" class="inline-block">
                  @csrf
                  @method('DELETE')
                  <button type="submit"
